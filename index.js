...
function removeArtist(artists, id) {
  console.log(artists[id].name + "is currently indexed at " + id);
  ...
  console.log(artists[id].name + " is now indexed at " + id);
...
function lotsOfArt(artists){
  ...
  let paints = [];
  for (let x = 0; x < artists.length; x++){
    if (artists[x].paintings > 100){
      paints.push(artists[x].name);
    }
  }
  console.log(paints);
return paints;
...
function addArtist(artists){
  ...
  let britt = {
    ...
    bio: "Lorem ipsum something or other, not concerned with what this says. yay (or use lorem ipsum) " 
  }
  console.log(britt);
  artists.push(britt);
